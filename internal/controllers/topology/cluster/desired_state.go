...
func (r *Reconciler) computeDesiredState(ctx context.Context, s *scope.Scope) (*scope.ClusterState, error) {
	...
	// Mark all the MachineDeployments and Machinepools that are currently upgrading.
	...
	// Checking a MachinePool if it is upgrading requires an available workload cluster.
	// As long as there is no initialized control plane, there will also be no machine pools
	// upgrading.
	if conditions.IsTrue(s.Current.Cluster, clusterv1.ControlPlaneInitializedCondition) {
